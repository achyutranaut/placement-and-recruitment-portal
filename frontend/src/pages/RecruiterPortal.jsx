...
export default function RecruiterPortal() {
  ...
  const filteredApplications = companyApplications.filter((app) => {
    ...
    const matchesTab =
      ...
      (activePipelineTab === "REJECTED"
        ? app.status === "REJECTED" || app.status === "DECLINED"
        : app.status === activePipelineTab);
    const matchesQuery =
      searchQuery === "" ||
  ...
  // Counts for pipeline tabs (ensuring every candidate belongs to exactly one bucket)
  const tabCounts = {
    ...
    REJECTED: companyApplications.filter((a) => a.status === "REJECTED" || a.status === "DECLINED").length,
  };

  // State Machine Handlers
  const handleSelect = async (app) => {
    try {
      await api.updateApplicationStatus(app.applicationId, "SELECTED");
      setNotification({
        type: "success",
        message: "Candidate " + app.studentName + " (" + app.studentId + ") marked as SELECTED.",
      });
      await loadData();
    } catch (err) {
                ...
                return (
                  <tr key={app.applicationId} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      {app.status === "INTERVIEWING" && app.scoreCompletion && (
                        <div className="mt-1">
                          {app.scoreCompletion.selectionEligible ? (
                            <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200">
                              <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                              Rounds Cleared ({app.scoreCompletion.completedRounds}/{app.scoreCompletion.requiredRounds})
                            </span>
                          ) : (
                            <span
                              className="inline-flex items-center gap-1 text-[10px] font-medium text-amber-800 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200"
                              title={app.scoreCompletion.readinessReason}
                            >
                              <Clock className="w-3 h-3 text-amber-600" />
                              {app.scoreCompletion.completedRounds}/{app.scoreCompletion.requiredRounds} Rounds
                            </span>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {app.status === "INTERVIEWING" && (
                          <>
                            ...
                            {/* Select Candidate: Enabled only when candidate has passed all required rounds */}
                            {(app.scoreCompletion?.selectionEligible || app.isSelectionEligible) ? (
                              <button
                                onClick={() => handleSelect(app)}
                                className="px-2.5 py-1 rounded bg-teal-600 text-white hover:bg-teal-700 text-xs font-semibold shadow-sm transition-colors"
                                title="All required interview rounds passed. Mark candidate as SELECTED."
                              >
                                Select Candidate
                              </button>
                            ) : (
                              <button
                                disabled
                                className="px-2.5 py-1 rounded bg-slate-100 text-slate-400 border border-slate-200 text-xs font-semibold cursor-not-allowed"
                                title="Complete all required interview rounds before selection."
                              >
                                Select Candidate
                              </button>
                            )}
                        ...
                        {/* State: ACCEPTED */}
                        {app.status === "ACCEPTED" && (
                          <span className="text-xs font-bold text-emerald-800 bg-emerald-50 border border-emerald-300 px-2.5 py-1 rounded flex items-center gap-1">
                        ...
                        {/* State: DECLINED */}
                        {app.status === "DECLINED" && (
                          <span className="text-xs font-medium text-slate-500 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded flex items-center gap-1">
